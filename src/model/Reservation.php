...
class Reservation
{
    ...
    // recupere tous les témoignages
    public function getAllTestimonies()
    {
        $db = new Database();
        $connection = $db->getConnection();

        $request = $connection->prepare('SELECT opinion.id AS opinion_id, opinion.created_at AS opinion_create_at, opinion.*, reservation.*, apartments.*, users.name AS user_name, users.email FROM opinion INNER JOIN reservation ON opinion.reservation_id = reservation.id INNER JOIN apartments ON reservation.apartment_id = apartments.id INNER JOIN users ON reservation.user_id = users.id');
        $request->execute();
        $results = $request->fetchAll(PDO::FETCH_ASSOC);
        return $results;
    }
    ...
    public function acceptTestimony()
    {
        ...
        $request = $connection->prepare('UPDATE `opinion` SET `state` = 1 WHERE id = :id');
        $request->bindParam(':id', $this->id);
